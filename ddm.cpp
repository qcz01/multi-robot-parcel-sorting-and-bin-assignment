...
void load_database() {
    ...
    std::cout<<"Data base loaded"<<std::endl;
    assert(ddm_database[1].size() == 36);
}

std::vector<std::vector<Node>> DdmSolver::solve(OneShotTask& task, Graph& g, int horizon) {
    ...
    while (true) {
        ...
        if (future_paths_all_empty || paths.size() > horizon) {
            ...
            //get the numer of goals reached
        ...
        for (size_t i = 0; i < task.num_robots; i++)
            if (future_paths[i].empty()) {
                ...
                if (suo) suo->add_path(future_paths[i], true);
            }
        ...
        if (!collided_robots.empty()) {
            for (auto collided_robot_pair : collided_robots) {
                ...
                if (subg.small) {
                    if (subg.hi.x - subg.lo.x ==2)  // Subgraph landscape placement
                        for (size_t i = 0; i < affected_robots.size(); i++) {
        ...
        for (auto r : stopped_robots) {
            ...
            if (future_paths[r].back() == paths.end()[-2][r]) {
                // TODO Update SUO
                // if (random_mode == 1)
                //     occupancy[future_paths[r].back().x]
                //              [future_paths[r].back().y] -= 1;
    ...
    while (true) {
        ...
        for (size_t i = 0; i < task.num_robots; i++){
            if (future_paths[i].empty()) {
                if(goal_ids[i]>=task.goals[i].size()){
                    future_paths[i].push_back(paths.back()[i]); //all the goals are finished
                }else{
        ...
        if (!collided_robots.empty()) {
            for (auto collided_robot_pair : collided_robots) {
                ...
                // std::cout<<paths.back()[r1]<<"-->"<<next_step[r1]<<"     "<<paths.back()[r2]<<"-->"<<next_step[r2]<<std::endl;
                ...
                for (size_t i = 0; i < affected_robots.size(); i++) {
                    ...
                    if (found_subgoal && it != potential_sub_goals.end()) {
                        ...
                        if (future_paths[affected_robots[i]].empty()) {
                            // TODO check if this is necessary
                            future_paths[affected_robots[i]].push_back(
                                task.goals[affected_robots[i]][goal_ids[affected_robots[i]]]);
                            // TODO STO: update path
                        }
                        continue;
                    }
