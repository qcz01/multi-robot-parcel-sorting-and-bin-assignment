   ...
   public:
    ...
    bool search(const std::vector<State>& initialStates,
        ...
        for (size_t i = 0; i < initialStates.size(); ++i) {
            size_t index = i;
            LowLevelEnvironment llenv(m_env, index, start.constraints[index],
                                      start.solution);
            LowLevelSearch_t lowLevel(llenv, 1.5);
            bool success =
                lowLevel.search(initialStates[index], start.solution[index]);
            if (!success) return false;
            start.cost += start.solution[index].cost;
            start.LB += start.solution[index].fmin;
        }
...
// update focal list
...
#else
            {
                ...
                if (bestCost > oldBestCost) {
                    // std::cout << "oldBestCost: " << oldBestCost << "
...
// check focal list/open list consistency
#ifdef CHECK_FOCAL_LIST
            {
                ...
                for (; iter != iterEnd; ++iter) {
                    ...
                    if (val <= bestCost * m_w) {
                        ...
                        if (std::find(focal.begin(), focal.end(), s.handle) !=
                            focal.end()) {
                            std::cout << "focalSet shouldn't have: " << s
                                      << std::endl;
                            mismatch = true;
                        }
                        // break;
            ...
            for (const auto& c : constraints) {
                ...
                newNode.id = id;
                // (optional) check that this constraint was not included
                // already std::cout << newNode.constraints[i] << std::endl;
                ...
                bool success =
                    lowLevel.search(initialStates[i], newNode.solution[i]);
...
// boost::heap::compare<compareFocalHeuristic> > focalSet_t;
    ...
    struct LowLevelEnvironment {
        ...
        void onExpandNode(const State& s, Cost fScore, Cost gScore) {
            // std::cout << "LL expand: " << s << " fScore: " << fScore << "
            // gScore: "
            // << gScore << std::endl;
